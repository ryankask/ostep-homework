...
class raid:
    def __init__(
        ...
        if (chunkSize % BLOCKSIZE) != 0:
            print(
                "chunksize (%d) must be multiple of blocksize (%d): %d"
                % (chunkSize, BLOCKSIZE, self.chunkSize % BLOCKSIZE)
            )
            exit(1)
    ...
    def stats(self, totalTime):
        for d in range(self.numDisks):
            ...
            if s[4] == totalTime:
                print(
                    "disk:%d  busy: %.2f  I/Os: %5d (sequential:%d nearly:%d random:%d)"
                    % (d, util, s[0], s[1], s[2], s[3])
                )
            elif s[4] == 0:
                print(
                    "disk:%d  busy:   %.2f  I/Os: %5d (sequential:%d nearly:%d random:%d)"
    ...
    def enqueue(self, addr, size, isWrite):
        ...
        if self.timing == False:
            if self.solve or self.reverse == False:
                if isWrite:
                    print(
                        "LOGICAL WRITE to  addr:%d size:%d" % (addr, size * BLOCKSIZE)
                    )
                else:
                    print(
                        "LOGICAL READ from addr:%d size:%d" % (addr, size * BLOCKSIZE)
                    )
                if self.solve == False:
    ...
    def bmap0(self, bnum):
        ...
        return (
            cnum % self.numDisks,
            int(int(cnum / self.numDisks) * self.chunkSize + coff),
        )
    ...
    def bmap1(self, bnum):
        ...
        return (
            disk,
            disk + 1,
            int(int(cnum / int(self.numDisks / 2))) * self.chunkSize + coff,
        )
    ...
    def bmap4(self, bnum):
        ...
        return (
            ...
            int(cnum / (self.numDisks - 1)) * self.chunkSize + coff,
        )
...
parser.add_option(
    ...
    "--workload",
    default="rand",
    help='either "rand" or "seq" workloads',
    action="store",
    type="string",
    dest="workload",
)
...
parser.add_option(
    ...
    "--randRange",
    default=10000,
    help='range of requests (when using "rand" workload)',
    action="store",
    type="int",
    dest="range",
)
...
if size % BLOCKSIZE != 0:
    print(
        "error: request size (%d) must be a multiple of BLOCKSIZE (%d)"
        % (size, BLOCKSIZE)
    )
    exit(1)
...
assert (
    options.level == 0 or options.level == 1 or options.level == 4 or options.level == 5
)
...
if options.level == 5 and options.raid5type != "LA" and options.raid5type != "LS":
    print(
        "Only two types of RAID-5 supported: left-asymmetric (LA) and left-symmetric (LS) (%s is not)"
        % options.raid5type
    )
    exit(1)
