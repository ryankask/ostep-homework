...
class ssd:
    ...
    def is_block_free(self, block):
        ...
        if (
            self.state[first_page] == self.STATE_INVALID
            or self.state[first_page] == self.STATE_ERASED
        ):
    ...
    def garbage_collect(self):
        ...
        for block in (
            x
            for y in (
                range(self.gc_current_block, self.num_blocks),
                range(0, self.gc_current_block),
            )
            for x in y
        ):
    ...
    def stats(self):
        ...
        print(
            "  Write count %d (%d failed)"
            % (self.logical_write_sum, self.logical_write_fail_sum)
        )
        print(
            "  Read count  %d (%d failed)"
            % (self.logical_read_sum, self.logical_read_fail_sum)
        )
        print(
            "  Trim count  %d (%d failed)"
            % (self.logical_trim_sum, self.logical_trim_fail_sum)
        )
        print("")
        print("Times")
        ...
        total_time = (
            self.physical_erase_sum * self.block_erase_time
            + self.physical_write_sum * self.page_program_time
            + self.physical_read_sum * self.page_read_time
        )
    ...
    def dump(self):
        ...
        for i in range(self.num_pages):
            if (
                self.state[i] == self.STATE_VALID
                and self.forward_map[self.reverse_map[i]] == i
            ):
                print("+", end="")
            else:
                print(" ", end="")
...
parser.add_option(
    ...
    "--num_cmds",
    default=10,
    help="number of commands to randomly generate",
    action="store",
    type="int",
    dest="num_cmds",
)
parser.add_option(
    ...
    "--op_percentages",
    default="40/50/10",
    help="if rand, percent of reads/writes/trims",
    action="store",
    type="string",
    dest="op_percentages",
)
parser.add_option(
    ...
    "--skew",
    default="",
    help="if non-empty, skew, e.g., 80/20: 80% of ops to 20% of blocks",
    action="store",
    type="string",
    dest="skew",
)
parser.add_option(
    ...
    "--skew_start",
    default=0,
    help="if --skew, skew after this many writes",
    action="store",
    type="int",
    dest="skew_start",
)
parser.add_option(
    ...
    "--read_fails",
    default=0,
    help="if rand, percent of reads that can fail",
    action="store",
    type="int",
    dest="read_fail",
)
parser.add_option(
    ...
    "--cmd_list",
    default="",
    help="comma-separated list of commands (e.g., r10,w20:a)",
    action="store",
    type="string",
    dest="cmd_list",
)
parser.add_option(
    ...
    "--ssd_type",
    default="direct",
    help="SSD type: ideal, direct, log",
    action="store",
    type="string",
    dest="ssd_type",
)
parser.add_option(
    ...
    "--logical_pages",
    default=50,
    help="number of logical pages in interface",
    action="store",
    type="int",
    dest="num_logical_pages",
)
parser.add_option(
    "-B",
    "--num_blocks",
    default=7,
    help="number of physical blocks in SSD",
    action="store",
    type="int",
    dest="num_blocks",
)
...
parser.add_option(
    ...
    "--high_water_mark",
    default=10,
    help="blocks used before gc trigger",
    action="store",
    type="int",
...
parser.add_option(
    ...
    "--low_water_mark",
    default=8,
    help="gc target before stopping gc",
    action="store",
    type="int",
    dest="low_water_mark",
)
...
parser.add_option(
    ...
    "--show_gc",
    default=False,
    help="show garbage collector behavior",
    action="store_true",
    dest="show_gc",
)
parser.add_option(
...
s = ssd(
    ssd_type=options.ssd_type,
    num_logical_pages=options.num_logical_pages,
    num_blocks=options.num_blocks,
    pages_per_block=options.pages_per_block,
    block_erase_time=float(options.erase_time),
    page_program_time=float(options.program_time),
    page_read_time=float(options.read_time),
    high_water_mark=options.high_water_mark,
    low_water_mark=options.low_water_mark,
    trace_gc=options.show_gc,
