...
class Request:
    ...
    def RequestLevel4Done(self, disk, timer):
        ...
        if (
            self.status[index] == REQ_PARITY_READ_PHASE_DONE
            and self.status[1 - index] == REQ_PARITY_READ_PHASE_DONE
        ):
...
class Raid:
    def __init__(
        ...
        if self.mapping == 0:
            ...
            for i in range(self.block_count):
                ...
                text_id = self.canvas.create_text(
                    rect_x + disk_width - disk_width / 2.0,
                    rect_y + 10,
                    text="%s" % i,
                    anchor="c",
                )
        ...
        for disk in range(self.disk_count):
            ...
            rect_y = 100
            head_id = self.canvas.create_rectangle(
                rect_x,
                rect_y,
                rect_x + self.head_width,
                rect_y + self.head_height,
                fill="black",
                outline="black",
            )
            ...
            self.head_position[disk] = {
                "x1": rect_x,
                "y1": rect_y,
                "x2": rect_x + self.head_width,
                "y2": rect_y + self.head_height,
            }
        ...
        for index in range(len(self.request_queue)):
            ...
            logical = request.GetLogicalAddress()
            self.request_queue_box_ids.append(
                self.canvas.create_rectangle(
                    x - sz, y - sz, x + sz, y + sz, fill="white", outline=""
                )
            )
            self.request_queue_text_ids.append(
                self.canvas.create_text(x, y, text=str(logical), anchor="c", font=font)
            )
            self.request_queue_count_ids.append(
    ...
    def StartRequest(
        self, disk, offset, logical_address, request, queue_index, fill_color
    ):
    ...
    def DoStripeScheduling(self, disk, index):
        ...
        logical = request.GetLogicalAddress()
        if (
            request.GetStatus(0) == REQ_NOT_STARTED
            and logical % self.disk_count == disk
        ):
    ...
    def DoMirrorScheduling(self, disk, index):
        ...
        if request.GetStatus(disk_index) == REQ_NOT_STARTED and (
            disk == disks[0] or disk == disks[1]
        ):
            offset = request.GetPhysicalOffset()
            request.MarkStart(self.timer)
            if request.GetType() == OP_READ:
    ...
    def DoRaid4Scheduling(self, disk, index):
        ...
        if (
            request.GetType() == OP_READ
            and request.GetStatus(0) == REQ_NOT_STARTED
            and logical % (self.disk_count - 1) == disk
        ):
            request.MarkStart(self.timer)
            request.SetStatus(0, REQ_DO_READ)
            offset = request.GetPhysicalOffset()
    ...
    def DoRaid5Scheduling(self, disk, index):
        ...
        if (
            request.GetType() == OP_READ
            and request.GetStatus(0) == REQ_NOT_STARTED
            and request.GetPhysicalDiskList()[0] == disk
        ):
            request.MarkStart(self.timer)
            request.SetStatus(0, REQ_DO_READ)
            offset = request.GetPhysicalOffset()
    ...
    def MoveHead(self, disk):
        ...
        self.canvas.coords(
            self.head_ids[disk],
            self.head_position[disk]["x1"],
            self.head_position[disk]["y1"],
            self.head_position[disk]["x2"],
            self.head_position[disk]["y2"],
        )
        return
    ...
    def DoneWithSeek(self, disk):
        ...
        if (
            self.GetHeadPosition(disk)
            == self.offset_to_ypos[request.GetPhysicalOffset()]
        ):
            return True
        return False
        ...
        for disk in range(self.disk_count):
            ...
            if self.disk_state[disk] == STATE_XFER:
                ...
                if self.DoneWithTransfer(disk):
                    ...
                    self.BlockSetColor(
                        disk,
                        self.current_target[disk].GetPhysicalOffset(),
    ...
    def PrintStats(self):
        ...
        print(
            "  Avg Latency: %.2f"
            % (float(self.latency_total) / float(self.logical_requests))
        )
        return
...
parser.add_option(
    ...
    "--mapping",
    default="0",
    help="0-striping, 1-mirroring, 4-raid4, 5-raid5",
    action="store",
    type="int",
    dest="mapping",
)
...
parser.add_option(
    ...
    "--balanced",
    default=True,
    help="If generating random requests, balance across disks",
    action="store_true",
    dest="balance",
)
parser.add_option(
    ...
    "--seek_speed",
    default="4",
    help="Speed of seek (1,2,4,5,10,20)",
    action="store",
    type="int",
    dest="seek_speed",
)
...
parser.add_option(
    ...
    "--delay",
    default=20,
    help="Animation delay; bigger is slower",
    action="store",
    type="int",
    dest="animate_delay",
)
...
d = Raid(
    ...
    addr_desc=options.addr_desc,
    addr=options.addr,
    disk_count=4,
    seek_speed=options.seek_speed,
    seed=options.seed,
    balance=options.balance,
    read_fraction=float(options.read_fraction),
    window=options.window,
    animate_delay=options.animate_delay,
)
