...
class cpu:
    ...
    def __init__(
        ...
        for r in self.regtrace:
            zassert(
                r in self.regnames,
                "Register %s cannot be traced because it does not exist" % r,
            )
    ...
    def getarg(self, arg):
        ...
        if tmp[0] == "$":
            ...
            if len(s) == 1:
                ...
                if tmp[0] != "(":
                    zassert(
                        s[0].strip().isdigit() == True,
                        "First number should be a digit [%s]" % s[0],
                    )
                    value = neg * int(s[0])
                else:
                    value = 0
    ...
    def removecommas(self, cline, inargs):
        ...
        for i in range(len(inargs)):
            if inargs[i] == "(":
                zassert(
                    inparen == False,
                    "cannot have nested parenthesis in argument [%s]" % cline,
                )
                inparen = True
            if inargs[i] == ")":
                zassert(
                    inparen == True,
                    "cannot have right parenthesis without first having left one [%s]"
                    % cline,
                )
                inparen = False
            if inparen == True:
        ...
        for line in fd:
            ...
            if cline[0] != ".":
                ...
                if opcode == "mov":
                    ...
                    zassert(
                        len(rtmp) == 2,
                        "mov: needs two args, separated by commas [%s]" % cline,
                    )
                    arg1 = rtmp[0].strip()
                    arg2 = rtmp[1].strip()
                    ...
                    if stype == "TYPE_MEMORY" and dtype == "TYPE_MEMORY":
                        ...
                        tmp = src.split(",")
                        assert len(tmp) == 4
                        self.memory[pc] = "self.move_m_to_r(%d, %d, %d, %d, %d)" % (
                            int(tmp[0]),
                            int(tmp[1]),
                            int(tmp[2]),
                            int(tmp[3]),
                        ...
                        tmp = dst.split(",")
                        assert len(tmp) == 4
                        self.memory[pc] = "self.move_r_to_m(%d, %d, %d, %d, %d)" % (
                            src,
                            int(tmp[0]),
                            int(tmp[1]),
                            int(tmp[2]),
                        ...
                        tmp = dst.split(",")
                        assert len(tmp) == 4
                        self.memory[pc] = "self.move_i_to_m(%d, %d, %d, %d, %d)" % (
                            src,
                            int(tmp[0]),
                            int(tmp[1]),
                            int(tmp[2]),
                    ...
                    zassert(
                        ...
                        "lea: needs two args, separated by commas [%s]" % cline,
                    )
                    arg1 = rtmp[0].strip()
                    arg2 = rtmp[1].strip()
                    (src, stype) = self.getarg(arg1)
                    ...
                    if stype == "TYPE_MEMORY" and dtype == "TYPE_REGISTER":
                        tmp = src.split(",")
                        assert len(tmp) == 4
                        self.memory[pc] = "self.lea_m_to_r(%d, %d, %d, %d, %d)" % (
                            int(tmp[0]),
                            int(tmp[1]),
                            int(tmp[2]),
                            int(tmp[3]),
                        ...
                        zassert(
                            False,
                            "malformed lea instruction (should be memory address source to register destination",
                        )
                elif opcode == "neg":
                    ...
                    if stype == "TYPE_REGISTER":
                        ...
                        tmp = src.split(",")
                        assert len(tmp) == 4
                        self.memory[pc] = "self.push_m(%d,%d,%d,%d)" % (
                            int(tmp[0]),
                            int(tmp[1]),
                            int(tmp[2]),
                            int(tmp[3]),
                    ...
                    zassert(
                        len(tmp) == 2 and len(rtmp) == 2,
                        "mul: needs two args, separated by commas [%s]" % cline,
                    )
                    arg1 = rtmp[0].strip()
                    arg2 = rtmp[1].strip()
                    (src, stype) = self.getarg(arg1)
                    ...
                    if stype == "TYPE_REGISTER" and dtype == "TYPE_MEMORY":
                        self.memory[pc] = "self.fetchadd(%d, %d, %d, %d)" % (
                            src,
                            int(tmp[0]),
                            int(tmp[1]),
                            int(tmp[2]),
                    ...
                    assert len(tmp) == 4
                    if stype == "TYPE_REGISTER" and dtype == "TYPE_MEMORY":
                        self.memory[pc] = "self.atomic_exchange(%d, %d, %d, %d)" % (
                            src,
                            int(tmp[0]),
                            int(tmp[1]),
                            int(tmp[2]),
                    ...
                    self.memory[pc] = (
                        "self.jump_lessthanorequal(%s)" % self.labels[targ]
                    )
                elif opcode == "jgt":
                    (targ, ttype) = self.getarg(tmp[1].strip())
                    zassert(
                    ...
                    self.memory[pc] = "self.jump_greaterthan(%d)" % int(
                        self.labels[targ]
                    )
                elif opcode == "jgte":
                    (targ, ttype) = self.getarg(tmp[1].strip())
                    ...
                    self.memory[pc] = (
                        "self.jump_greaterthanorequal(%s)" % self.labels[targ]
                    )
                elif opcode == "nop":
                ...
                if self.verbose:
                    print(
                        "pc:%d LOADING %20s --> %s"
                        % (pc, self.pmemory[pc], self.memory[pc])
                    )
        ...
        if cctrace == True:
            for c in self.condlist:
                if self.compute:
                    ...
                    print("? ", end=" ")
        if (
            len(self.memtrace) > 0 or len(self.regtrace) > 0 or cctrace == True
        ) and newline == True:
            print("")
        return
    ...
    def run(self, procs, intfreq, intrand):
        ...
        while True:
            if (
                self.headercount > 0
                and self.icount % self.headercount == 0
                and self.icount > 0
            ):
                print("")
                self.print_headers(procs)
            ...
            if interrupt == 0 or rc == -2:
                ...
                if procs.ismanual() == False or (
                    procs.ismanual() == True and curr != next
                ):
                    self.print_trace(False)
                    for i in range(procs.getnum()):
                        print("------ Interrupt ------ ", end=" ")
...
class proclist:
    ...
    def finalize(self, procsched):
        ...
        for i in range(len(procsched)):
            ...
            if p >= self.getnum():
                print(
                    "bad schedule: cannot include a thread that does not exist (%d)" % p
                )
                exit(1)
...
parser.add_option(
    ...
    "--procsched",
    default="",
    help="control exactly which thread runs when",
    action="store",
    type="string",
    dest="procsched",
)
...
parser.add_option(
    ...
    "--loadaddr",
    default=1000,
    help="address where to load code",
    action="store",
    type="int",
    dest="loadaddr",
)
...
parser.add_option(
    ...
    "--headercount",
    default=-1,
    help="how often to print a row header",
    action="store",
    type="int",
    dest="headercount",
)
...
cpu = cpu(
    memsize, memtrace, regtrace, cctrace, options.solve, verbose, printstats, hdrcount
)
...
if printstats:
    ...
    print(
        "STATS:: Emulation Rate  %.2f kinst/sec" % (float(ic) / float(t2 - t1) / 1000.0)
    )
