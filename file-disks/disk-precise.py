...
class Disk:
    def __init__(
        ...
        if self.seekSpeed > 1 and self.trackWidth % self.seekSpeed != 0:
            print(
                "Seek speed (%f) must divide evenly into track width (%f)"
    ...
    def DoneWithTransfer(self):
        ...
        if self.RadiallyCloseTo(
            self.angle,
            Decimal((self.blockToAngleMap[self.currentBlock] + angleOffset) % 360),
        ):
    ...
    def UpdateWindow(self):
        if (
            ...
            and self.currWindow < len(self.requestQueue)
        ):
            self.currWindow += 1
            if self.graphics:
                self.DrawWindow()
...
d = Disk(
    ...
    seekSpeed=Decimal(options.seekSpeed),
    rotateSpeed=Decimal(options.rotateSpeed),
    skew=options.skew,
