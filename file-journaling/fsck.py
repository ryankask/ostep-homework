...
class fs:
    ...
    def dump(self):
        ...
        for i in range(0, self.numInodes):
            ...
            if ftype == "free":
                ...
                print(
                    "[%s a:%s r:%d]"
                    % (ftype, self.inodes[i].getAddr(), self.inodes[i].getRefCnt()),
                    end=" ",
                )
        print("")
    ...
    def writeFile(self, tfile, data):
        ...
        dprint(
            "writeFile: inum:%d cursize:%d refcnt:%d"
...
parser.add_option(
    ...
    "--seed",
    default=0,
    help="first random seed (for a filesystem)",
    action="store",
    type="int",
    dest="seed",
)
parser.add_option(
    ...
    "--seedCorrupt",
    default=0,
    help="second random seed (for corruptions)",
    action="store",
    type="int",
    dest="seedCorrupt",
)
...
parser.add_option(
    ...
    "--dontCorrupt",
    default=False,
    help="actually corrupt file system",
    action="store_true",
    dest="dontCorrupt",
)
